...
mydmam.async.request("broker","action",c,function(e){console.log(d,b,e);});},onActionButtonClick:function(d,c){var b={job_key:d.key,order:c};
mydmam.async.request("broker","action",b,function(e){console.log(d,c,e);});},componentWillMount:function(){this.updateJobList();
...
return(React.createElement(mydmam.async.PageHeaderTitle,{title:i18n("Job list"),fluid:"true"},React.createElement(a.NavTabs,{selected_tab:this.state.selected_tab,onActionTabClick:this.onActionTabClick,onTabChange:this.onTabChange,joblist:this.state.joblist,action_avaliable:b}),React.createElement(a.JobListCartridges,{joblist:this.state.joblist,selected_tab:this.state.selected_tab,onActionButtonClick:this.onActionButtonClick,action_avaliable:b})));
...
return(React.createElement("li",{key:"action",ref:"tab",className:f},React.createElement("a",{className:"dropdown-toggle",href:location.hash,onClick:this.onDropDownActionClick},"For each ",React.createElement("b",{className:"caret"})),React.createElement("ul",{className:"dropdown-menu"},e)));
...
if(j[m]){l.push(j[m]);}}}e.push(React.createElement(a.JobCartridge,{key:d.key,job:d,version:this.state.version,required_jobs:l,action_avaliable:this.props.action_avaliable,onActionButtonClick:this.props.onActionButtonClick}));
}var g=null;if(i){g=(React.createElement(mydmam.async.AlertBox,{title:"Too many jobs to list here"},"You can actually see only ",c.length," jobs."));
...
},onClickDoNothing:function(b){b.stopPropagation();},render:function(){var g=this.props.job;
var w=this.props.required_jobs;var i=null;var o=null;var p=null;var n=null;var c=null;
if(this.state.stacked){var q=null;if(g.max_execution_time<(1000*3600*24)){if(g.max_execution_time>(3600*1000)){q=(React.createElement("span",{className:"label"},i18n("manager.jobs.max_execution_time_hrs",Math.round((g.max_execution_time/(3600*1000))))));
}else{q=(React.createElement("span",{className:"label"},i18n("manager.jobs.max_execution_time_sec",(g.max_execution_time/1000))));
}}i=(React.createElement("div",null,React.createElement("div",{style:{marginTop:5}},React.createElement(mydmam.async.pathindex.reactDate,{i18nlabel:"create_date",date:g.create_date,style:{marginLeft:0}})),React.createElement("div",{style:{marginTop:4}},React.createElement("span",{className:"label label-info"},React.createElement("i",{className:"icon-cog icon-white"})," Created by ",g.instance_status_creator_key)),React.createElement("div",{style:{marginTop:4}},"Class creator: ",React.createElement(mydmam.async.JavaClassNameLink,{javaclass:g.creator,version:this.props.version})),React.createElement("div",{style:{marginTop:4,marginBottom:8}},q)));
var k=[];if(g.start_date>0){k.push(React.createElement("div",{style:{marginTop:5},key:"sdate"},React.createElement(mydmam.async.pathindex.reactDate,{i18nlabel:"start_date",date:g.start_date})));
if(g.end_date>0){k.push(React.createElement("div",{style:{marginTop:5},key:"edate"},React.createElement(mydmam.async.pathindex.reactDate,{i18nlabel:"end_date",date:g.end_date})));
k.push(React.createElement("div",{style:{marginTop:5},key:"deltadate"},React.createElement("span",{className:"label",style:{marginLeft:5}},React.createElement("i",{className:"icon-time icon-white"})," ",(g.end_date-g.start_date)/1000+" sec.")));
k.push(React.createElement("div",{style:{marginTop:5},key:"sinceedate"},React.createElement(mydmam.async.pathindex.reactSinceDate,{i18nlabel:"end_date",date:g.end_date})));
}}o=(React.createElement("div",null,k));p=(React.createElement(mydmam.async.JavaStackTrace,{processing_error:g.processing_error,version:this.props.version}));
var v=null;var f=JSON.stringify(g.context.content,null," ");if(f!="{}"){v=(React.createElement("code",{className:"json",onClick:this.onClickDoNothing},React.createElement("i",{className:"icon-indent-left"}),React.createElement("span",{className:"jsontitle"}," Données du contexte : "),f));
}var b=null;if(g.context.neededstorages){var h="Target storage:";if(g.context.neededstorages.length>1){h="Target storages:";
}b=(React.createElement("span",null,h," ",React.createElement("span",{className:"badge badge-warning"},React.createElement("i",{className:"icon-hdd icon-white"})," ",g.context.neededstorages.join(", "))));
}var r=null;if(g.context.hookednames){var h="Hooked name:";if(g.context.hookednames.length>1){h="Hooked names:";
}r=(React.createElement("span",null,h," ",React.createElement("span",{className:"badge badge-inverse"},React.createElement("i",{className:"icon-tags icon-white"})," ",g.context.hookednames.join(", "))));
}c=(React.createElement("div",{style:{marginBottom:7}},React.createElement("div",{style:{marginTop:5}},"Context Class: ",React.createElement(mydmam.async.JavaClassNameLink,{javaclass:g.context.classname,version:this.props.version})),React.createElement("div",{style:{marginTop:5}},b),React.createElement("div",{style:{marginTop:5}},r),React.createElement("div",{style:{marginTop:7}},v)));
var l=(React.createElement("div",{style:{marginTop:5}},"Job ref: ",React.createElement("span",null,a.displayKey(g.key,true))));
if(g.worker_reference!=null&g.worker_class!=null){n=(React.createElement("div",{className:"span7"},React.createElement("div",{style:{marginTop:5}},"Worker ref: ",React.createElement("span",null,a.displayKey(g.worker_reference,true))),React.createElement("div",{style:{marginTop:5}},"Class executor: ",React.createElement(mydmam.async.JavaClassNameLink,{javaclass:g.worker_class,version:this.props.version})),l));
}else{n=(React.createElement("div",{className:"span7"},l));}}var e=null;if(g.delete_after_completed){e=(React.createElement("span",{className:"label label-inverse"},i18n("manager.jobs.delete_after_completed")));
}var t=null;if(g.priority>0){var u=null;if(g.urgent){u=(React.createElement("i",{className:"icon-warning-sign icon-white"}));
}t=(React.createElement("span",{className:"badge badge-important"},u," ",i18n("manager.jobs.priority",g.priority)));
}var d=null;if(g.status==="WAITING"|g.status==="TOO_OLD"){d=(React.createElement(mydmam.async.pathindex.reactDate,{i18nlabel:"expiration_date",date:g.expiration_date}));
}else{if(g.status==="PROCESSING"|g.status==="DONE"|g.status==="STOPPED"|g.status==="TOO_LONG_DURATION"|g.status==="CANCELED"|g.status==="POSTPONED"|g.status==="ERROR"){d=(React.createElement(a.JobProgression,{job:g}));
}}var m=null;if(g.instance_status_executor_key){if(this.state.stacked){m=(React.createElement("span",{className:"label label-info"},React.createElement("i",{className:"icon-cog icon-white"})," by ",g.instance_status_executor_key));
}else{m=(React.createElement("span",{className:"label label-info"},React.createElement("i",{className:"icon-cog icon-white"})," by ",g.instance_status_executor_key.substring(0,g.instance_status_executor_key.indexOf("#"))));
}}var j=classNames("row-fluid","hover-focus",{stacked:this.state.stacked});var s=null;
if(this.props.action_avaliable){s=(React.createElement(a.JobCartridgeActionButtons,{status:g.status,stacked:this.state.stacked,onActionButtonClick:this.onActionButtonClick}));
}return(React.createElement("div",{className:j,onClick:this.onToogleCartridgeSize},React.createElement("div",{className:"span3 nomargin"},React.createElement("strong",null,g.name),i),React.createElement("div",{className:"span3 nomargin"},React.createElement(mydmam.async.pathindex.reactSinceDate,{i18nlabel:"update_date",date:g.update_date})," ",e," ",t,o),React.createElement("div",{className:"span3 nomargin"},d,c,p),React.createElement("div",{className:"span3 nomargin"},m,n,s)));
