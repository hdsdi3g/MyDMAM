...
document.body.style.cursor="default";},componentWillMount:function(){mydmam.async.request("instances","appversion",null,function(b){mydmam.async.appversion=b;
}.bind(this));this.updateJobList();},componentDidMount:function(){this.setState({interval:setInterval(this.updateJobList,3000)});
...
}});a.JobListCartridges=React.createClass({displayName:"JobListCartridges",render:function(){var j=this.props.joblist;
var h=this.props.selected_tab;var i=false;var c=[];for(var f in this.props.joblist){var d=this.props.joblist[f];
...
if(j[m]){l.push(j[m]);}else{l.push({key:m});}}}e.push(React.createElement(a.JobCartridge,{key:d.key,job:d,required_jobs:l,action_avaliable:this.props.action_avaliable,onActionButtonClick:this.props.onActionButtonClick}));
...
}var d=c;if(c.indexOf(":")>-1){d=c.substring(c.lastIndexOf(":")+1,c.lastIndexOf(":")+9)+".";
}else{if(c.indexOf("#")>-1){d=c.substring(c.indexOf("#")+1,c.length);}}if(b){return(React.createElement("abbr",{title:c},React.createElement("code",null,React.createElement("i",{className:"icon-barcode"})," ",d)));
...
}});a.displayContext=function(e){var f=null;if(e.content!=null){var d=JSON.stringify(e.content,null," ");
if(d!="{}"){f=(React.createElement("code",{className:"json",onClick:this.onClickDoNothing},React.createElement("i",{className:"icon-indent-left"}),React.createElement("span",{className:"jsontitle"}," ",i18n("manager.jobs.context")),d));
}f=(React.createElement("div",{style:{marginTop:7}},f));}var i=null;if(e.neededstorages){var j="manager.jobs.targetstorage";
if(e.neededstorages.length>1){j="manager.jobs.targetstorages";}var b=[];for(var c in e.neededstorages){if(e.neededstorages.length>3){b.push(React.createElement("div",{key:c},React.createElement("span",{className:"badge badge-warning",style:{marginLeft:8}},React.createElement("i",{className:"icon-hdd icon-white"})," ",e.neededstorages[c])));
}else{b.push(React.createElement("span",{className:"badge badge-warning",key:c,style:{marginLeft:8}},React.createElement("i",{className:"icon-hdd icon-white"})," ",e.neededstorages[c]));
}}i=(React.createElement("span",null,i18n(j),b));}var k=null;if(e.hookednames){var j="manager.jobs.hookedname";
if(e.hookednames.length>1){j="manager.jobs.hookednames";}var g=[];for(var h in e.hookednames){if(e.hookednames.length>1){b.push(React.createElement("div",{key:c},React.createElement("span",{className:"badge badge-inverse",style:{marginLeft:8}},React.createElement("i",{className:"icon-tags icon-white"})," ",e.hookednames[h])));
}else{b.push(React.createElement("span",{className:"badge badge-inverse",key:c,style:{marginLeft:8}},React.createElement("i",{className:"icon-tags icon-white"})," ",e.hookednames[h]));
}}k=(React.createElement("span",null,i18n(j),g));}return(React.createElement("div",{style:{marginBottom:7}},React.createElement("div",{style:{marginTop:5}},i18n("manager.jobs.contextclass")," ",React.createElement(mydmam.async.JavaClassNameLink,{javaclass:e.classname})),React.createElement("div",{style:{marginTop:5}},i),React.createElement("div",{style:{marginTop:5}},k),f));
...
},onClickDoNothing:function(b){b.stopPropagation();},render:function(){var f=this.props.job;
var w=this.props.required_jobs;var i=null;var h=null;var q=null;var r=null;var p=null;
var b=null;if(this.state.stacked){if(w.length>0){var g=[];for(var e in w){if(w[e].status){var l=a.status[w[e].status];
var m=classNames("badge",l.badge_class);g.push(React.createElement("div",{key:e},React.createElement("i",{className:"icon-hand-right",style:{marginLeft:5}}),React.createElement("strong",{style:{marginLeft:8,marginRight:8}},w[e].name),React.createElement("span",{className:m,style:{marginRight:8}},i18n(l.i18n_tab_name)),a.displayKey(w[e].key,true)));
}else{g.push(React.createElement("div",{key:e},React.createElement("i",{className:"icon-trash",style:{marginLeft:5}}),React.createElement("strong",{style:{marginLeft:8,marginRight:8}},i18n("manager.jobs.deletedjob"))));
}}h=(React.createElement("div",{className:"well well-small"},React.createElement("i",{className:"icon-tasks"})," ",i18n("manager.jobs.require"),React.createElement("div",{style:{marginLeft:8}},g)));
}var s=null;if(f.max_execution_time<(1000*3600*24)){if(f.max_execution_time>(3600*1000)){s=(React.createElement("span",{className:"label"},i18n("manager.jobs.max_execution_time_hrs",Math.round((f.max_execution_time/(3600*1000))))));
}else{s=(React.createElement("span",{className:"label"},i18n("manager.jobs.max_execution_time_sec",(f.max_execution_time/1000))));
}}i=(React.createElement("div",null,React.createElement("div",{style:{marginTop:5}},React.createElement(mydmam.async.pathindex.reactDate,{i18nlabel:"manager.jobs.createdate",date:f.create_date,style:{marginLeft:0}})),React.createElement("div",{style:{marginTop:4}},React.createElement("span",{className:"label label-info"},React.createElement("i",{className:"icon-cog icon-white"})," ",i18n("manager.jobs.createdbysimple",f.instance_status_creator_key))),React.createElement("div",{style:{marginTop:4}},i18n("manager.jobs.classcreator")," ",React.createElement(mydmam.async.JavaClassNameLink,{javaclass:f.creator})),React.createElement("div",{style:{marginTop:4,marginBottom:8}},s)));
var k=[];if(f.start_date>0){k.push(React.createElement("div",{style:{marginTop:5},key:"sdate"},React.createElement(mydmam.async.pathindex.reactDate,{i18nlabel:"manager.jobs.start_date",date:f.start_date})));
if(f.end_date>0){k.push(React.createElement("div",{style:{marginTop:5},key:"edate"},React.createElement(mydmam.async.pathindex.reactDate,{i18nlabel:"manager.jobs.end_date",date:f.end_date})));
k.push(React.createElement("div",{style:{marginTop:5},key:"deltadate"},React.createElement("span",{className:"label",style:{marginLeft:5}},React.createElement("i",{className:"icon-time icon-white"})," ",i18n("manager.jobs.duration",Math.round((f.end_date-f.start_date)/1000)))));
k.push(React.createElement("div",{style:{marginTop:5},key:"sinceedate"},React.createElement(mydmam.async.pathindex.reactSinceDate,{i18nlabel:"manager.jobs.end_date_for",date:f.end_date})));
}}q=(React.createElement("div",null,k));r=(React.createElement(mydmam.async.JavaStackTrace,{processing_error:f.processing_error}));
b=a.displayContext(f.context);var n=(React.createElement("div",{style:{marginTop:5}},i18n("manager.jobs.jref")," ",React.createElement("span",null,a.displayKey(f.key,true))));
if(f.worker_reference!=null&f.worker_class!=null){p=(React.createElement("div",{className:"span7"},React.createElement("div",{style:{marginTop:5}},i18n("manager.jobs.wref")," ",React.createElement("span",null,a.displayKey(f.worker_reference,true))),React.createElement("div",{style:{marginTop:5}},i18n("manager.jobs.classexec")," ",React.createElement(mydmam.async.JavaClassNameLink,{javaclass:f.worker_class})),n));
...
}}var j=classNames("row-fluid","hover-focus",{stacked:this.state.stacked});var t=null;
if(this.props.action_avaliable){t=(React.createElement(a.JobCartridgeActionButtons,{status:f.status,stacked:this.state.stacked,onActionButtonClick:this.onActionButtonClick}));
}return(React.createElement("div",{className:j,onClick:this.onToogleCartridgeSize},React.createElement("div",{className:"span3 nomargin"},React.createElement("strong",null,f.name),i,h),React.createElement("div",{className:"span3 nomargin"},React.createElement(mydmam.async.pathindex.reactSinceDate,{i18nlabel:"manager.jobs.update_date",date:f.update_date})," ",d," ",u,q),React.createElement("div",{className:"span3 nomargin"},c,b,r),React.createElement("div",{className:"span3 nomargin"},o,p,t)));
