...
ftpserver.MainPage = React.createClass({displayName: "MainPage",
	render: function() {
		var content = [];

		content.push({
			i18nlabel: "ftpserver.tab.activity",
			content: (React.createElement(ftpserver.ActivityList, null))
		});

		content.push({
			i18nlabel: "ftpserver.tab.users",
			content: (React.createElement(ftpserver.UserList, null))
		});

		if (ftpserver.hasUserAdminRights()) {
			content.push({
				i18nlabel: "ftpserver.tab.adduser",
				content: (React.createElement(ftpserver.AddUser, {params: {}})),
			});
		}

		return (React.createElement(mydmam.async.PageHeaderTitle, {title: i18n("ftpserver.pagename"), fluid: "true", tabs: content}));
	},
});

mydmam.routes.push("ftpserver-MainPage", "ftpserver", ftpserver.MainPage, [{name: "ftpserver", verb: "allusers"}]);	

ftpserver.UserList = React.createClass({displayName: "UserList",
	...
	render: function() {
		...
		for (pos_user in users) {
			...
			table_lines.push(React.createElement(ftpserver.UserLine, {
				key: pos_user, 
				user: user, 
			...
			if (present_in_last_activity_toogle) {
				table_lines.push(React.createElement("tr", {key: pos_user + "_lat"}, 
					React.createElement("td", {colSpan: "5"}, React.createElement(ftpserver.ActivityList, {user_id: user.user_id}))
				));
			}
		}
		...
		if (table_lines.length === 0) {
			return (React.createElement("div", null, 
				React.createElement(mydmam.async.AlertInfoBox, null, 
		...
		return (React.createElement("div", null, 
			...
			React.createElement("form", {className: "form-inline pull-right"}, 
				React.createElement("button", {className: "btn btn-small", onClick: this.onWantRefreshAll, style: {marginRight: "1em"}}, 
					React.createElement("i", {className: "icon-refresh"}), " ", i18n("ftpserver.userlist.refresh")
				), 
				React.createElement("label", {className: "checkbox"}, React.createElement("input", {type: "checkbox", onClick: this.onUnLockDelete, ref: "cb_unlockdelete"}), 
					i18n("ftpserver.userlist.unlockdelete")
				)
			), 
			React.createElement("table", {className: "table table-striped table-hover table-bordered table-condensed"}, 
				React.createElement("thead", null, 
					React.createElement("tr", null, 
						React.createElement("th", null, 
							i18n("ftpserver.userlist.table.title.user"), 
							React.createElement(ButtonSort, {onChangeState: this.onChangeColSort, colname: "user", order: this.getCurrentColSort("user")})
						), 
						React.createElement("th", null, 
							i18n("ftpserver.userlist.table.title.group"), 
							React.createElement(ButtonSort, {onChangeState: this.onChangeColSort, colname: "group", order: this.getCurrentColSort("group")})
						), 
						React.createElement("th", null, 
							i18n("ftpserver.userlist.table.title.createdat"), 
							React.createElement(ButtonSort, {onChangeState: this.onChangeColSort, colname: "created", order: this.getCurrentColSort("created")})
						), 
						React.createElement("th", null, 
							i18n("ftpserver.userlist.table.title.updatedat"), 
							React.createElement(ButtonSort, {onChangeState: this.onChangeColSort, colname: "updated", order: this.getCurrentColSort("updated")})
						), 
						React.createElement("th", null, 
							i18n("ftpserver.userlist.table.title.lastloginat"), 
							React.createElement(ButtonSort, {onChangeState: this.onChangeColSort, colname: "lastlogin", order: this.getCurrentColSort("lastlogin")})
						), 
						React.createElement("th", null, 
							React.createElement(ButtonSort, {onChangeState: this.onChangeColSort, colname: "enabled", order: this.getCurrentColSort("enabled")})
						)
					)
				), 
...
ftpserver.UserLine = React.createClass({displayName: "UserLine",
	...
	render: function() {
		...
		if (ftpserver.hasUserAdminRights()) {
			...
			if (mydmam.manager.url_ftpserver_export_user_sessions) {
				...
				if (user.last_login > 0) {
					...
					ftp_export = (React.createElement("a", {className: btn_export_classes, style: {marginRight: 5}, href: url}, 
						React.createElement("i", {className: "icon-download"}), " ", 
						i18n("ftpserver.userlist.table.btnsessions")
					));
				} else {
					ftp_export = (React.createElement("button", {className: btn_export_classes, style: {marginRight: 5}}, 
						React.createElement("i", {className: "icon-download"}), " ", 
						i18n("ftpserver.userlist.table.btnsessions")
					));
				}
			...
			btns_admin = (React.createElement("span", {style: {marginLeft: 5}}, 
				React.createElement("a", {className: "btn btn-mini btn-warning", style: {marginRight: 5}, href: "#ftpserver/edit/" + this.props.user.user_id}, 
					React.createElement("i", {className: "icon-edit icon-white"}), " ", 
					i18n("ftpserver.userlist.table.btnchpassword")
				), 
				ftp_export, 
				React.createElement(mydmam.async.BtnDelete, {label: i18n("ftpserver.userlist.table.delete"), enabled: this.props.delete_enabled, onClickDelete: this.onDelete, reference: user.user_id})
			));
		}

		var btn_last_activity_toogle = null;
		if (user.last_login > 0) {
			btn_last_activity_toogle = (React.createElement("span", {className: "pull-right"}, React.createElement(ftpserver.BtnLastActivityToogle, {
				user: user, 
		...
		var first_case_content = (React.createElement("span", null, 
			React.createElement("strong", null, user.domain), " :: ", user.user_name, 
			btn_last_activity_toogle
		));

		var first_case = (React.createElement("td", null, first_case_content));
		if (this.props.present_in_last_activity_toogle) {
			first_case = (React.createElement("td", {rowSpan: "2"}, first_case_content));
		}

		return (React.createElement("tr", null, 
			first_case, 
			React.createElement("td", null, 
				React.createElement("span", {className: "label label-inverse"}, user.group_name)
			), 
			React.createElement("td", null, React.createElement(mydmam.async.pathindex.reactDate, {date: user.create_date, style: {}})), 
			React.createElement("td", null, React.createElement(mydmam.async.pathindex.reactDate, {date: user.update_date, style: {}})), 
			React.createElement("td", null, React.createElement(mydmam.async.pathindex.reactDate, {date: user.last_login, style: {}})), 
			React.createElement("td", null, 
...
ftpserver.BtnLastActivityToogle = React.createClass({displayName: "BtnLastActivityToogle",
	...
	render: function() {
		...
		return (React.createElement("button", {className: class_name, onClick: this.onLastActivityToogle}, 
...
// Source hash: d209cfebde77fb9fcce2ff819e3f235d
