...
class MessageRegisterStore implements MessageDStoreMapper {
	...
	MessageRegisterStore(Action action, String database, Class<?> sync_class, RunningState running_state) {
		this.action = action;
		if (action == null) {
	...
	private MessageRegisterStore() {
	}
	
	static MessageRegisterStore fromJson(JsonElement json) {
		return MyDMAM.gson_kit.getGson().fromJson(json, MessageRegisterStore.class);
	}
