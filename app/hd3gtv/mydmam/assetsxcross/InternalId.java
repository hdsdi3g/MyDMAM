...
public class InternalId {
	...
	public static InternalId create(InterplayAsset asset, String path_referer, String interplay_uri_referer) {
		/*if (getInternalidByMOBId(asset.getMobID()) != null) {
		}*/
	...
	public void save() {
		...
		if (_id == null) {
			...
			/*if (internalid_by_mobid == null) {
				internalid_by_mobid = new HashMap<>();
			}
			internalid_by_mobid.put(mob_id, _id);*/
		} else {
			create.get();
		}
	}
	...
	// private static HashMap<String, String> internalid_by_mobid;
	
	/*public static String getInternalidByMOBId(String mob_id) {
		if (internalid_by_mobid == null) {
			internalid_by_mobid = new HashMap<>();
			try {
				Elasticsearch.createCrawlerReader().setIndices(ES_INDEX).setTypes(ES_TYPE).allReader(hit -> {
					String _mob_id = (String) hit.getSource().get("mob_id");
					if (_mob_id != null) {
						internalid_by_mobid.put(_mob_id, hit.getId());
					}
					return true;
				});
			} catch (IndexMissingException e) {
			} catch (Exception e) {
				Loggers.AssetsXCross.error("Can't search in ES", e);
			}
		}
		return internalid_by_mobid.get(mob_id);
	}*/
